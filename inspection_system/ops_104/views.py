...
def dashboard(request):
    ...
    if request.user.is_superuser:
        project = Project.objects.filter(name__icontains='104').first()
    else:
        try:
    ...
    if request.user.is_superuser:
        ...
        if allowed_service_codes.exists():
            allowed_vehicles = Vehicle.objects.filter(service_code__in=allowed_service_codes)
    ...
    # Filter recent complaints for inspectors to only show their own
    if profile and profile.role and profile.role.name.lower().strip() == 'inspection':
        # Fallback to inspection__inspector for older records where created_by is NULL
        from django.db.models import Q
        recent_complaints_qs = Complaint.objects.filter(
            Q(created_by=request.user) | Q(inspection__inspector=request.user)
        ).select_related('category', 'inspection__vehicle', 'inspection__inspector__profile__role').order_by('-created_at')[:10]
    else:
        recent_complaints_qs = complaint_stats_qs.select_related('category', 'inspection__vehicle', 'inspection__inspector__profile__role').order_by('-created_at')[:5]
    
    recent_complaints = list(recent_complaints_qs)
    
    # Initialize display time for all roles
    for c in recent_complaints:
        c.display_created_at = c.created_at

    # Role-locked Dynamic 24h Spoofing for Inspection Role (only for re-raised items if external)
    if profile and profile.role and profile.role.name.lower().strip() == 'inspection':
        now = timezone.now()
        for c in recent_complaints:
            # Check if this record has an external predecessor (re-marked)
            # Re-marked items should show the 24h spoofed time if requested
            others = Complaint.objects.filter(
                inspection__vehicle=c.inspection.vehicle,
                category=c.category,
                question=c.question,
                created_at__lt=c.created_at
            ).exclude(id=c.id).exclude(created_by=request.user).exclude(status='closed')
            
            if others.exists():
                # This was a re-marking of an older external issue
                # User wants 24h manipulations shown here too
                if (now - c.created_at).total_seconds() > 86400:
    ...
    return render(request, 'ops_104/dashboard.html', context)
...
def perform_inspection(request):
    ...
    if allowed_service_codes.exists():
        available_vehicles = Vehicle.objects.filter(service_code__in=allowed_service_codes).select_related('district', 'service_code')
    ...
    vehicle = None
    if v_id:
        vehicle = get_object_or_404(available_vehicles, id=v_id)
    elif available_vehicles.count() == 1:
    ...
    if vehicle and not request.user.is_superuser and profile and profile.role:
        ...
        if all_active.exists():
            if user_role == 'inspection':
                # Show only the most recent active complaint per item
                seen_items = set()
                active_list = []
                now = timezone.now()
    ...
    if request.method == 'POST':
        if not vehicle:
            ...
            return redirect('ops_104:inspection')
    ...
    return render(request, 'ops_104/inspection_form.html', context)
...
def manage_checklist(request):
    ...
    if request.method == 'POST':
        ...
        return redirect('ops_104:manage_checklist')
    ...
    return render(request, 'ops_104/manage_checklist.html', context)

@login_required
def complaint_list(request):
    user = request.user
    profile = getattr(user, 'profile', None)
    user_role = (profile.role.name.lower().strip() if profile and profile.role else '').strip()
    ...
    # Inspector view: Complaints raised by them
    from django.db.models import Q
    my_raised_complaints = Complaint.objects.filter(
        Q(created_by=user) | Q(inspection__inspector=user)
    ).select_related('inspection__vehicle', 'category').order_by('-created_at')
    
    # Identification Logic and 24h Spoofing for Inspectors
    now = timezone.now()
    for comp in my_raised_complaints:
        # 1. Date Fix / Spoofing Logic
        comp.display_created_at = comp.created_at
        
        # Check if this was a re-raise of an old external complaint
        others = Complaint.objects.filter(
            inspection__vehicle=comp.inspection.vehicle,
            category=comp.category,
            question=comp.question,
            created_at__lt=comp.created_at
        ).exclude(id=comp.id).exclude(created_by=user).exclude(status='closed')
        
        comp.is_remarked = others.exists()
        
        # Apply spoofing ONLY for re-raised items if they are older than 24h
        if comp.is_remarked and (now - comp.created_at).total_seconds() > 86400:
            comp.display_created_at = now - timezone.timedelta(hours=24)
    ...
    return render(request, 'ops_104/complaint_list.html', context)
...
def resolve_complaint(request, complaint_id):
    ...
    return redirect('ops_104:complaint_list')
